...
| data_binding | external_system | 外部系统 | VARCHAR(50) | 是 | - | ERP, MES, WMS, IOT | - | 外部系统类型 |
| data_binding | external_id | 外部ID | VARCHAR(100) | 是 | - | - | - | 外部系统标识 |
| data_binding | binding_type | 绑定类型 | VARCHAR(50) | 是 | - | EQUIPMENT, MATERIAL, ORDER, STATUS | - | 绑定数据类型 |
| data_binding | mapping_rules | 映射规则 | JSON | 是 | {} | - | - | 字段映射规则 |
| data_binding | sync_config | 同步配置 | JSON | 是 | {} | - | - | 同步频率、方式等 |
| data_binding | status | 状态 | VARCHAR(20) | 是 | "ACTIVE" | ACTIVE, INACTIVE, ERROR | - | 绑定状态 |
...
| 字典项 | 值 | 文案 | 说明 | 是否可配置 | 生效范围 |
|--------|----|------|------|-----------|---------|
| 设备 | EQUIPMENT | 设备 | 设备主数据绑定 | 是 | 数据绑定 |
| 物料 | MATERIAL | 物料 | 物料数据绑定 | 是 | 数据绑定 |
| 订单 | ORDER | 订单 | 生产订单绑定 | 是 | 数据绑定 |
| 状态 | STATUS | 状态 | 实时状态绑定 | 是 | 数据绑定 |
